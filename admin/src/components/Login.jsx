...
const Login = ({ setToken }) => {
  ...
  const onSendOtpSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`${backendUrl}/api/admin/send-reset-otp`, { email });
      if (response.data.success) {
        toast.success(response.data.message || "OTP Sent successfully!");
        setCurrentState('Reset'); // Advance to inputting the OTP
      } else {
        toast.error(response.data.message);
      }
    } catch (error) {
  ...
  // 3. Finalize Reset Password Verification
        ...
        <div className="w-full max-w-md bg-white/10 md:bg-transparent backdrop-blur-xl md:backdrop-blur-none p-8 rounded-2xl border border-white/20 md:border-none shadow-2xl md:shadow-none transition-all duration-500">
          ...
          <div className="mb-8 space-y-2">
            <h1 className="text-3xl font-serif tracking-tight text-white md:text-[#061b0e] transition-all duration-300">
              {currentState === 'Login' && "Admin Gateway"}
              {currentState === 'Forgot' && "Reset Request"}
              {currentState === 'Reset' && "Establish Password"}
            </h1>
            <p className="text-sm text-stone-200 md:text-stone-500 font-light">
              {currentState === 'Login' && "Sign in with your master credentials to open the administrative panel dashboard."}
              {currentState === 'Forgot' && "Enter your register admin address. We'll send an authentication security code."}
              {currentState === 'Reset' && "Provide the token delivered to your inbox followed by your updated credentials."}
            </p>
          </div>
          ...
          {/* STATE B: FORGOT PASSWORD (OTP REQUEST) */}
          {currentState === 'Forgot' && (
            <form onSubmit={onSendOtpSubmit} className="space-y-5">
              ...
              >
                <span>{loading ? "Sending..." : "Transmit One-Time Token"}</span>
              ...
              >
                <CornerUpLeft className="w-3.5 h-3.5" /> Return to Login
              </button>
            </form>
          )}

          {/* STATE C: RESET PASSWORD FORM */}
          {currentState === 'Reset' && (
            <form onSubmit={onResetPasswordSubmit} className="space-y-5">
              ...
              <button
                ...
                className="w-full py-3.5 mt-4 bg-white text-[#061b0e] md:bg-[#061b0e] md:text-white hover:bg-stone-100 md:hover:bg-[#13301d] font-semibold md:font-medium tracking-wide rounded-xl shadow-md transition-all active:scale-[0.99] flex items-center justify-center gap-2 group disabled:opacity-50"
              >
                <span>{loading ? "Overwriting..." : "Apply Structural Reset"}</span>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 text-xs text-stone-300 md:text-stone-500 hover:text-white md:hover:text-stone-800 mt-2 transition-colors cursor-pointer"
              >
                <CornerUpLeft className="w-3.5 h-3.5" /> Request a new OTP
              </button>
            </form>
          )}
